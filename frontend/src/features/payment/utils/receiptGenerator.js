...
export function generatePaymentReceipt(payment) {
  ...
  // Color Palette
  const primaryColor = [245, 158, 11]; // Amber #f59e0b
  ...
  doc.setFontSize(22);
  doc.text('E-BOOK LIBRARY SYSTEM', 20, 25);
  ...
  doc.text(`Payment Date: ${payment?.payment_date ? payment.payment_date.slice(0, 19).replace('T', ' ') : new Date().toLocaleString()}`, 20, 70);
  ...
  // Paid Fee Table
  const tableHeaders = [['Material Title', 'Fee Type', 'Paid Amount']];
  const tableData = [
    [
  ...
  doc.setFontSize(14);
  doc.text(`$${Number(payment?.amount || 0).toFixed(2)}`, 160, finalY + 7);

  // Footer Branding and Signatures
  ...
  const safeTitle = String(payment?.material_title || 'receipt').toLowerCase().replace(/[^a-z0-9]/g, '_');
